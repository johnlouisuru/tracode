...
<x-app-layout x-data="{ open: false }">
    ...
    <x-slot name="header">
      ...
      <nav class="font-semibold text-xl text-gray-800">
        ...
        <div class="float-right">
          Csharp Score: {{Auth::user()->csharpscore}}
            </div>
      </nav>
  </x-slot>
  ...
  <body style="background-color: #90ebf0">
    ...
    <div style="z-index: 100; margin-top: -15%;">
     ...
     <center> <h1>{{$csharpquizs->title}}</h1><a onclick="cond2()" href="#" style="text-decoration: none; color: black; " class=" h5">&#x1F4A1;</a></center>
  </div>
  <div class="card-body bg-white" style="border-left: 1px solid black; border-right: 1px solid black; border-bottom: 1px solid black; ">
    <pre style=" text-align: left; border: none!important; border-radius: 0%; color: black;">{{$csharpquizs->question}}</pre>
  </div>
</div>
    </div>
  </div>
  <div style="margin-left: 12%; margin-top: -8%;">
        ...
        <input id="admin" placeholder="admin key word" value="{{$csharpquizs->answerkey}}" hidden>
        <input id="admin2" placeholder="admin key word2"  value="{{$csharpquizs->answerkey2}}" hidden>
        <input id="admin3" placeholder="admin key word3"  value="{{$csharpquizs->answerkey3}}" hidden>

        <h5>Enter Syntax Code</h5>
          
<textarea id="source" class ="lined" style=" width: calc(53% - 7px); resize: none;" rows="15" placeholder="Enter Syntax Code Here" spellcheck="false" > using System;
      ...
      <br><br>
        ...
        <textarea readonly id="output" style=" margin-top: -50%; width: 30%; background-color: black; color: white; resize: none; padding-bottom: 2.6%;" rows="17" placeholder="This will display the output of your code" ></textarea>
          ...
          <script type="text/javascript">
            ...
            API_KEY = "b4f5082e41mshed67ea24159c051p1c9bfbjsn13c2b90acb8d";
...
$(function() {
    ...
    <div class="popup" id="popup-1">
      ...
      <div style="margin-top:5%; margin-right:6%;"class="content">
        ...
        <h3> You gained points</h3>
        {!! Form::open(['action'=>['App\Http\Controllers\CsharpScoringControllerinter@store'],'method'=>'POST','class'=>'pull-right']) !!}
        {{Form::hidden('iduser',Auth::user()->id)}}
        {{Form::hidden('idquizz',$csharpquizs->id)}}
        {{Form::hidden('difficulty', $csharpquizs->category)}}
