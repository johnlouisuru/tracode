      ...
      <div class="container">
        ...
        <div style="z-index: 100; margin-top: 5%;">
          <div class="card m-4 "style="background-color: transparent; border: none!important;" data-aos="fade-left" data-aos-duration="500">
            <div class="card-body" style="background-color: white; margin-top: 5%;border: 1px solid white; ">
            <h1 style="border-bottom: 1px solid white;">Basic PHP</h1>
          <h4>In this category, it focuses on basic scripting codes that is used to run programs for this language containing examples to show how codes work together.</h4>
        </div>
        <div class="card-body" data-aos="fade-left" data-aos-duration="1000">
        <ul class="list-group">
            @if(count($phptabs)>0)
            @foreach ($phptabs as $ptab)
            <li class="list-group-item" style="background-color: #ffbf00; border: none!important; border-radius: 0%; color: black;"><a href="/phpbasictrylang/{{$ptab->id}}" style="color: black;">{{$ptab->title}}</a>  <sub class="float-sm-right"> {{$ptab->category}}</sub></li>
            @endforeach
