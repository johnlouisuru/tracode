...
<x-app-layout x-data="{ open: false }">
  ...
  <body style="background-color: #90ebf0">
    ...
    <div style="z-index: 100; margin-top: -15%;">
     ...
     <center> <h1>{{$javaquizs->title}}</h1><a onclick="cond2()" href="#" style="text-decoration: none; color: black; " class=" h5">&#x1F4A1;</a></center>
  </div>
  <div class="card-body bg-white" style="border-left: 1px solid black; border-right: 1px solid black; border-bottom: 1px solid black; ">
    <pre style=" text-align: left; border: none!important; border-radius: 0%; color: black;">{{$javaquizs->question}}</pre>
  </div>
</div>
    </div>
    </div>
    <div style="margin-left: 12%; margin-top: -8%;">
        ...
        <input id="admin" placeholder="admin key word" value="{{$javaquizs->answerkey}}" hidden>
        <input id="admin2" placeholder="admin key word2"  value="{{$javaquizs->answerkey2}}" hidden>
        <input id="admin3" placeholder="admin key word3"  value="{{$javaquizs->answerkey3}}" hidden>

        <h5>Enter Syntax Code</h5>
          
<textarea id="source" class ="lined" style=" width: calc(53% - 7px); resize: none;" rows="15" placeholder="Enter Syntax Code Here" spellcheck="false" >public class Main {
...
$(function() {
    ...
    <div class="popup" id="popup-1">
      ...
      <div style="margin-top:5%; margin-right:6%;"class="content">
        ...
        {!! Form::open(['action'=>['App\Http\Controllers\JavaScoringControllerinter@store'],'method'=>'POST','class'=>'pull-right']) !!}
        {{Form::hidden('iduser',Auth::user()->id)}}
        {{Form::hidden('idquizz',$javaquizs->id)}}
        {{Form::hidden('difficulty', $javaquizs->category)}}
